...
class BGASwipeBackShadowView extends FrameLayout {
     ...
     * 阴影资源 id
     */
    private int mShadowResId = R.drawable.shape_shadow;
    /**
     * 阴影区域的透明度是否根据滑动的距离渐变
     */
    ...
    void bindPreActivity() {
        ...
        if (mPreActivity == null) {
            ...
            if (preActivity != null) {
                ...
                if (containsWebView((ViewGroup) mPreContentView)) {
                } else {
     ...
     * 该 ViewGroup 中是否包含导致多次 draw 后应用崩溃的 View
     */
    private boolean containsWebView(ViewGroup viewGroup) {
        int childCount = viewGroup.getChildCount();
        View childView;
        for (int i = 0; i < childCount; i++) {
            childView = viewGroup.getChildAt(i);
            if (childView instanceof WebView) {
                ...
                if (containsWebView((ViewGroup) childView)) {
                    return true;
                }
            }
        }
